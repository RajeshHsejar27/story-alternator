...
def _get_a2a_urls() -> tuple[str, str]:
...
async def _get_card(client: httpx.AsyncClient) -> AgentCard:
    global _card
    if _card is None:
        a2a_base, a2a_card_url = _get_a2a_urls()
...
async def chat(req: Request):
    ...
    async with httpx.AsyncClient(headers=_auth_headers(), timeout=120) as client:
        ...
        async for event in a2a_client.send_message(msg):
            print("DEBUG A2A EVENT:", type(event), event, flush=True)
...
async def upload_book(
    ...
    prompt = (
        f"I am uploading a custom book titled '{title}' by {author}.\n"
        f"Book text excerpt:\n{text[:3000]}\n\n"
        f"Please run `analyze_uploaded_book` for title '{title}' by '{author}' with this text, "
        f"and present 3 compelling choices to pivot the narrative."
    )
    ...
    if not parts:
        parts = [{"kind": "text", "text": f"Book '{title}' uploaded successfully."}]
    return JSONResponse({"parts": parts, "title": title, "author": author})
